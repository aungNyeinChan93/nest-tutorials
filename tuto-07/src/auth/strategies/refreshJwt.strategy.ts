...
export class RefreshJwtStrategy extends PassportStrategy(Strategy, 'refresh-jwt') {
    ...
    async validate(req: Request, payload: JwtAuthPayload) {
        ...
        const isUserVerify = await this.authService.verifyRefreshToken({ user, refreshToken })
        if (!isUserVerify) throw new UnauthorizedException('refresh token is invalid')
