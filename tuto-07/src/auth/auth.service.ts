...
export class AuthService {
  ...
  async signIn(user: ValidatedUser) {
    ...
    const updateSuccess = await this.userService.updateHashRefreshToken({
      userId: user?.id,
      hashRefreshToken,
    });
  ...
  async verifyRefreshToken({
    ...
    try {
      const isSuccess = await argon.verify(
        user?.hashRefreshToken as string,
        refreshToken,
      );
      if (!isSuccess)
      ...
      logger.error(
        error instanceof Error ? error?.message : 'cerifyToken fail!',
      );
    }
  }
  ...
  async signOut(userId: number) {
    const isSuccess = await this.userService.updateHashRefreshToken({
      userId,
